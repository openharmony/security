...
由于安全处理流程涉及到多个环节和对应的职责，SC的成员会被赋予一些特定的角色，以更加明确的承担这些环节的职责。以下是各个角色的定义，这些角色会定期进行轮换，以保证每一个人都有机会了解OpenHarmony的安全处理机制
...
+ 如需要在私有安全库进行开发，修复负责人将使修复团队可以访问OpenHarmonyXXXXXX中的私有安全存储库，以开发修复程序。
...
注意：OpenHarmony私有安全存储库由SC拥有。组织管理由sig-infrastructure完成。
...
+ 修复负责人将补丁程序通过电子邮件发送给dev@openharmony.io，以便发行商可以提前准备，并在发行版发布之日向用户提供版本。
...
+ 修复负责人将该问题的回顾过程发送到security@openharmony.io，包括有关每个人的详细信息，过程时间表的执行情况，与引入该问题相关的PR链接，以及对响应和发布的任何处理或建议。
+ 鼓励发行经理和修复团队将自己的反馈意见发送到security@openharmony.io。诚实的批判是我们改善社区的唯一途径。
...
该列表用于向OpenHarmony的发行商提供安全相关可操作的信息，请参考[发行商列表](private-distributors-list.md)
